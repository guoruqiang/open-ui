<script lang="ts">
	...
	let AUDIO_SPEECH_PREVIEW_BASE_URL = '';
	let AUDIO_SPEECH_PREVIEW_API_KEYS = '';
	...
	const updateConfigHandler = async () => {
		const res = await updateAudioConfig(localStorage.token, {
			tts: {
				...
				AUDIO_SPEECH_PREVIEW_BASE_URL: AUDIO_SPEECH_PREVIEW_BASE_URL,
				AUDIO_SPEECH_PREVIEW_API_KEYS: AUDIO_SPEECH_PREVIEW_API_KEYS
			},
			stt: {
	...
	onMount(async () => {
		...
		if (res) {
			...
			AUDIO_SPEECH_PREVIEW_BASE_URL = res.tts.AUDIO_SPEECH_PREVIEW_BASE_URL;
			AUDIO_SPEECH_PREVIEW_API_KEYS = res.tts.AUDIO_SPEECH_PREVIEW_API_KEYS;
				...
				<div>
					...
					<div class="mt-1 flex gap-2 mb-1">
						<input
							...
							bind:value={AUDIO_SPEECH_PREVIEW_BASE_URL}
							required
						/>

						<SensitiveInput
							placeholder={$i18n.t('Preview Speech API Keys')}
							bind:value={AUDIO_SPEECH_PREVIEW_API_KEYS}
						/>
					</div>
				</div>
